<?php
	...
	elseif($cpass==$srow['password']){
		...
		mysqli_query($conn,"update `user` set username='$username', password='$cpass' where userid='".$_SESSION['staffid']."'");
		mysqli_query($conn,"insert into activitylog (userid,action,activity_date) values ('".$_SESSION['staffid']."','Update account',NOW())");
		?>
		<script>
